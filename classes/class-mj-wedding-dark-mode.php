...
class mj_wedding_Dark_Mode {
	...
	public function __construct() {
		...
		// Add HTML classes.
		add_filter( 'mjwedding_html_classes', array( $this, 'html_classes' ) );
	...
	public function customizer_controls( $wp_customize ) {
		...
		if ( is_object( $colors_section ) ) {
			$colors_section->title = __( 'Colors & Dark Mode', 'mjwedding' );
		}

		// Custom notice control.
		include_once get_theme_file_path( 'classes/class-mj_wedding-customize-notice-control.php' ); // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		...
		$wp_customize->add_control(
			...
			array(
				'type'            => 'checkbox',
				'section'         => 'colors',
				'label'           => esc_html__( 'Dark Mode support', 'mjwedding' ),
				'priority'        => 110,
	...
	public function the_html( $attrs = array() ) {
		...
		printf(
			...
			esc_html__( 'Dark Mode: %s', 'mjwedding' ),
		...
		<style>
			...
			#dark-mode-toggler > span::before {
				content: '<?php esc_attr_e( 'Off', 'mjwedding' ); ?>';
			}
			#dark-mode-toggler[aria-pressed="true"] > span::before {
				content: '<?php esc_attr_e( 'On', 'mjwedding' ); ?>';
			}
	...
	public function add_privacy_policy_content() {
		...
		$content = '<p class="privacy-policy-tutorial">' . __( 'M & J Wedding uses LocalStorage when Dark Mode support is enabled.', 'mjwedding' ) . '</p>'
				. '<strong class="privacy-policy-tutorial">' . __( 'Suggested text:', 'mjwedding' ) . '</strong> '
				. __( 'This website uses LocalStorage to save the setting when Dark Mode support is turned on or off.<br> LocalStorage is necessary for the setting to work and is only used when a user clicks on the Dark Mode button.<br> No data is saved in the database or transferred.', 'mjwedding' );
		wp_add_privacy_policy_content( 'M & J Wedding', wp_kses_post( wpautop( $content, false ) ) );
	}
